...
def test_user_is_added():
    created_user_name = create_new_user()
    peoples = requests.get(BASE_URL).json()
    newly_created_user = search_created_user(peoples, created_user_name)[0]

    get_url = f'{BASE_URL}/{newly_created_user["name"]}'
    response = requests.get(get_url)
    print(response)
    # is_new_user_created = search_created_user(peoples, unique_name)
...
def search_created_user(peoples, name):
    return [person for person in peoples if person['name'] == name]