...
class Tests(Testdata):
    ...
    def test_new_user_successfully_created(self):
        ...
        assert_that(new_user).is_equal_to((self.unique_name, requests.codes.created))
    ...
    def  test_invalid_guser_422_code(self):
        ...
        assert_that(invalid_user.status_code).is_equal_to(requests.codes.unprocessable_entity)
    ...
    def  test_invalid_guser_401_code(self):
        ...
        assert_that(unauthorized_user.status_code).is_equal_to(requests.codes.unauthorized)

    #404 status code:  Not found
    def  test_invalid_guser_404_code(self):
        get_data = APIRequests()
        not_existing_user = get_data.get_guser_with_not_existing_id()
        assert_that(not_existing_user.status_code).is_equal_to(requests.codes.not_found)
    
    #400 status code: Bad Request
    def  test_invalid_guser_400_code(self):
        get_data = APIRequests()
        invalid_user = get_data.create_guser_with_invalid_endpoint()
        assert_that(invalid_user.status_code).is_equal_to(requests.codes.bad_request)
    ...
    def test_get_added_user(self):
        ...
        assert_that(get_added_user).is_equal_to((self.unique_name, requests.codes.ok))
