...
class APIRequests(Testdata):
    ...
    def create_new_guser(self):
        self.post_response = requests.post(
            self.base_url, data=self.payload, headers=self.header)
    ...
    def create_new_guser_with_invalid_data(self):
        self.invalid_post_response = requests.post(
            self.base_url, data=self.payload, headers=self.invalidheaders)
        return self.invalid_post_response

    def create_new_guser_without_authorization(self):
        self.not_authorized_user = requests.post(
            self.base_url, data=self.payload)
        return self.not_authorized_user

    def get_guser_with_not_existing_id(self):
        self.not_existing_user = requests.get(
            url=f'{BASE_URL}/{self.not_existing_id}', data=self.payload)
        return self.not_existing_user

    def create_guser_with_invalid_endpoint(self):
        self.invalid_user = requests.post(
            url=f'{BASE_URL}/{self.test}', data=self.payload, headers=self.header)
        return self.invalid_user

    def get_added_user(self):
        self.get_all_users = requests.get(
            self.base_url, headers=self.header).json()
        global added_user
        added_user = [
            self.person for self.person in self.get_all_users if self.person['name'] == self.unique_name][0]
    ...
    def change_created_user(self):
        ...
        put_response = requests.put(
            self.updated_url, data=self.updated_payload, headers=self.header)
